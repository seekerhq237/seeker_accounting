"""
Patch script: rewrites the _run_merge method in backup_merge_service.py
with correct column names matching the actual DB schema.

Run: python scripts/_patch_merge_service.py
"""
import textwrap

SRC = "src/seeker_accounting/modules/administration/services/backup_merge_service.py"

# Read original (partially truncated — header only survives)
with open(SRC, "r", encoding="utf-8") as f:
    header_lines = f.readlines()

# The file was truncated — only the header remains (up to end of _merge_db)
# We'll append _run_merge + helpers after the header

# The new _run_merge method body (properly indented at 4 spaces for method, 8 for body)
NEW_RUN_MERGE = '''\
    def _run_merge(
        self,
        src: Connection,
...
with open(SRC, "w", encoding="utf-8") as f:
    for line in header_lines:
        f.write(line)
    f.write("\n")
    f.write(NEW_RUN_MERGE)
    f.write("\n")
    f.write(HELPER_METHODS)

print(f"OK Patched {SRC}")
print(f"  Header: {len(header_lines)} lines")
print(f"  New _run_merge: {NEW_RUN_MERGE.count(chr(10))} lines")
print(f"  Helpers: {HELPER_METHODS.count(chr(10))} lines")
