...
from seeker_accounting.modules.purchases.dto.purchase_credit_note_commands import PurchaseCreditNoteLineCommand
...
_HEADERS = ("#", "Description", "Amount", "Tax Code", "Expense Account", "Line Total")
_EDITABLE_COLS = (_COL_DESCRIPTION, _COL_AMOUNT, _COL_TAX_CODE, _COL_ACCOUNT)


class PurchaseCreditNoteLinesGrid(QFrame):
    """Editable line-items grid for Purchase Credit Notes."""
    ...
    def __init__(self, service_registry: ServiceRegistry, company_id: int, parent: QWidget | None = None) -> None:
        ...
        self._amount_delegate = NumericDelegate(decimals=2, parent=self._table)
        ...
        self._table.setItemDelegateForColumn(_COL_AMOUNT, self._amount_delegate)
        ...
        hh.setSectionResizeMode(_COL_ROW_NUM, QHeaderView.ResizeMode.Fixed)
        hh.resizeSection(_COL_ROW_NUM, 32)
        hh.setSectionResizeMode(_COL_DESCRIPTION, QHeaderView.ResizeMode.Stretch)
        hh.setSectionResizeMode(_COL_AMOUNT, QHeaderView.ResizeMode.Fixed)
        hh.resizeSection(_COL_AMOUNT, 110)
        hh.setSectionResizeMode(_COL_TAX_CODE, QHeaderView.ResizeMode.Fixed)
        hh.resizeSection(_COL_TAX_CODE, 130)
        hh.setSectionResizeMode(_COL_ACCOUNT, QHeaderView.ResizeMode.Fixed)
        hh.resizeSection(_COL_ACCOUNT, 180)
        hh.setSectionResizeMode(_COL_LINE_TOTAL, QHeaderView.ResizeMode.Fixed)
        hh.resizeSection(_COL_LINE_TOTAL, 100)
    ...
    def _load_reference_data(self) -> None:
        try:
            ...
            _log.warning("PCN lines: failed to load accounts", exc_info=True)
            self._accounts = []
        ...
        try:
            ...
            _log.warning("PCN lines: failed to load tax codes", exc_info=True)
            self._tax_codes = []
    ...
    def set_lines(self, lines: list[PurchaseCreditNoteLineDTO]) -> None:
        self._updating = True
        try:
    ...
    def get_line_commands(self) -> list[PurchaseCreditNoteLineCommand]:
        commands: list[PurchaseCreditNoteLineCommand] = []
        for row in range(self._model.rowCount()):
            description = (self._model.data(self._model.index(row, _COL_DESCRIPTION)) or "").strip()
            if not description:
                continue
            amount = self._get_decimal(row, _COL_AMOUNT) or Decimal("0.00")
            ...
            if not isinstance(tax_code_id, int) or tax_code_id <= 0:
                tax_code_id = None
            expense_account_id = self._model.data(self._model.index(row, _COL_ACCOUNT), Qt.ItemDataRole.UserRole)
            if not isinstance(expense_account_id, int) or expense_account_id <= 0:
    ...
    def _add_empty_row(self) -> None:
        self._append_row(description="", amount=None, tax_code_id=None, expense_account_id=None, line_total=Decimal("0.00"))
        self._refresh_empty_state()
        new_row = self._model.rowCount() - 1
        self._table.setCurrentIndex(self._model.index(new_row, _COL_DESCRIPTION))
    ...
    def _append_row(
        ...
        try:
            ...
            amt_item = QStandardItem()
            if amount is not None:
                amt_item.setData(_format_number(amount), Qt.ItemDataRole.DisplayRole)
                amt_item.setData(amount, Qt.ItemDataRole.UserRole)
            items.append(amt_item)
            ...
            if expense_account_id and expense_account_id > 0:
                acct_item.setData(self._account_display(expense_account_id), Qt.ItemDataRole.DisplayRole)
                acct_item.setData(expense_account_id, Qt.ItemDataRole.UserRole)
    ...
    def _calculate_row_amounts(self, row: int) -> tuple[Decimal, Decimal, Decimal]:
        amount = self._get_decimal(row, _COL_AMOUNT) or Decimal("0")
        tax_code_id = self._model.data(self._model.index(row, _COL_TAX_CODE), Qt.ItemDataRole.UserRole)
        tax_rate = Decimal("0")
        if isinstance(tax_code_id, int) and tax_code_id > 0:
            tax_rate = self._tax_rates_by_id.get(tax_code_id, Decimal("0"))
        tax = (amount * tax_rate / Decimal("100")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    ...
    def _on_data_changed(self, top_left: QModelIndex, _bottom_right: QModelIndex) -> None:
        if self._updating:
            return
        col = top_left.column()
        if col in (_COL_AMOUNT, _COL_TAX_CODE):
