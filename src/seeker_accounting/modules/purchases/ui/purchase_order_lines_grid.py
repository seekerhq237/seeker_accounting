...
from seeker_accounting.modules.purchases.dto.purchase_order_commands import PurchaseOrderLineCommand
from seeker_accounting.modules.purchases.dto.purchase_order_dto import PurchaseOrderLineDTO
...
_HEADERS = ("#", "Description", "Qty", "Unit Cost", "Disc %", "Tax Code", "Expense Account", "Line Total")

_EDITABLE_COLS = (_COL_DESCRIPTION, _COL_QTY, _COL_UNIT_COST, _COL_DISCOUNT, _COL_TAX_CODE, _COL_ACCOUNT)


class PurchaseOrderLinesGrid(QFrame):
    """Editable line-items grid for Purchase Orders.

    Public API:
        set_lines(lines)
        get_line_commands() -> list[PurchaseOrderLineCommand]
    ...
    def __init__(
        ...
        title = QLabel("Order Lines", header_row)
        ...
        header.resizeSection(_COL_UNIT_COST, 90)
    ...
    def _load_reference_data(self) -> None:
        try:
            ...
            _log.warning("PO lines: failed to load accounts", exc_info=True)
            self._accounts = []
        ...
        try:
            ...
            _log.warning("PO lines: failed to load tax codes", exc_info=True)
            self._tax_codes = []
            self._tax_rates_by_id = {}
    ...
    def set_lines(self, lines: tuple[PurchaseOrderLineDTO, ...]) -> None:
        self._updating = True
        try:
    ...
    def get_line_commands(self) -> list[PurchaseOrderLineCommand]:
        commands: list[PurchaseOrderLineCommand] = []
    ...
    def _calculate_row_amounts(self, row: int) -> tuple[Decimal, Decimal, Decimal]:
        ...
        cost = self._get_decimal(row, _COL_UNIT_COST) or Decimal("0")
        ...
        subtotal = (qty * cost * disc_factor).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    ...
    def _on_data_changed(self, top_left: QModelIndex, _bottom_right: QModelIndex) -> None:
        ...
        row = top_left.row()
        col = top_left.column()
        if col in (_COL_QTY, _COL_UNIT_COST, _COL_DISCOUNT, _COL_TAX_CODE):
            self._refresh_row_total(row)
        self.lines_changed.emit()
