...
from seeker_accounting.modules.purchases.dto.purchase_bill_commands import PurchaseBillLineCommand
from seeker_accounting.modules.purchases.dto.purchase_bill_dto import PurchaseBillLineDTO
...
_HEADERS = ("#", "Description", "Qty", "Unit Cost", "Tax Code", "Account", "Line Total")

# Ordered list of editable columns for arrow-key navigation
_EDITABLE_COLS = (_COL_DESCRIPTION, _COL_QTY, _COL_UNIT_COST, _COL_TAX_CODE, _COL_ACCOUNT)


class PurchaseBillLinesGrid(QFrame):
    """Editable line-items grid for Purchase Bills.

    Public API (unchanged from previous version):
        set_lines(lines)
        get_line_commands() -> list[PurchaseBillLineCommand]
    ...
    def __init__(
        ...
        title = QLabel("Bill Lines", header_row)
        ...
        # Enable mouse tracking so :hover QSS on persistent editors updates
        # in real-time as the cursor moves (not after the cursor has left).
        ...
        header.resizeSection(_COL_TAX_CODE, 140)
        ...
        header.resizeSection(_COL_LINE_TOTAL, 110)
    ...
    def set_lines(self, lines: tuple[PurchaseBillLineDTO, ...]) -> None:
        self._updating = True
        try:
    ...
    def get_line_commands(self) -> list[PurchaseBillLineCommand]:
        commands: list[PurchaseBillLineCommand] = []
        for row in range(self._model.rowCount()):
            ...
            expense_account_id = self._model.data(
    ...
    def _append_row(
        ...
        try:
            ...
            # Col 0: row number (painted by delegate, data ignored)
            ...
            if unit_cost is not None:
                cost_item.setData(_format_number(unit_cost), Qt.ItemDataRole.DisplayRole)
            ...
            if expense_account_id and expense_account_id > 0:
                display = self._account_display(expense_account_id)
    ...
    def _open_row_editors(self, row: int) -> None:
        ...
        for col in _EDITABLE_COLS:
            ...
            # Install arrow-key navigation filter on each editor widget
    ...
    def _remove_selected_row(self) -> None:
        ...
        if not index.isValid():
            return
        row = index.row()
        # Close persistent editors before removing
    ...
    def _calculate_row_amounts(self, row: int) -> tuple[Decimal, Decimal, Decimal]:
        ...
        resolved_cost = unit_cost if unit_cost is not None else Decimal("0.00")
        subtotal = (resolved_qty * resolved_cost).quantize(
    ...
    def _on_data_changed(
        ...
        if self._updating:
            return
        # Recalculate line totals if qty, unit cost, or tax code changed
        for row in range(top_left.row(), bottom_right.row() + 1):
            col = top_left.column()
            if col in (_COL_QTY, _COL_UNIT_COST, _COL_TAX_CODE):
                self._refresh_row_total(row)
        self.lines_changed.emit()
    ...
    def _refresh_empty_state(self) -> None:
        ...
        if not has_rows:
            # Center the overlay within the table viewport
    ...
    def eventFilter(self, obj: QObject, event: QEvent) -> bool:
        ...
        # Find which cell this editor belongs to
