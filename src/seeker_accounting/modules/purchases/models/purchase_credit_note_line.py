...
class PurchaseCreditNoteLine(TimestampMixin, Base):
    __tablename__ = "purchase_credit_note_lines"
    __table_args__ = (
        UniqueConstraint("purchase_credit_note_id", "line_number"),
        Index("ix_purchase_credit_note_lines_project_id", "project_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    purchase_credit_note_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("purchase_credit_notes.id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    tax_details: Mapped[list["PurchaseCreditNoteLineTax"]] = relationship(
        "PurchaseCreditNoteLineTax",
        back_populates="purchase_credit_note_line",
