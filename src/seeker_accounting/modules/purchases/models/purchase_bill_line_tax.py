...
class PurchaseBillLineTax(TimestampMixin, Base):
    __tablename__ = "purchase_bill_line_taxes"
    __table_args__ = (
        Index("ix_purchase_bill_line_taxes_line_id", "purchase_bill_line_id"),
        Index("ix_purchase_bill_line_taxes_tax_code_id", "tax_code_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    purchase_bill_line_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("purchase_bill_lines.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    purchase_bill_line: Mapped["PurchaseBillLine"] = relationship(
        "PurchaseBillLine", back_populates="tax_details"
    )
