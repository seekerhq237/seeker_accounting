...
from seeker_accounting.modules.reporting.ui.dialogs.stock_movement_window import (
...
class StockValuationWindow(QFrame):
    """Focused operational valuation report using posted inventory truth."""
    ...
    def __init__(
        ...
        self._report_service = service_registry.stock_valuation_report_service
        self._movement_service = service_registry.stock_movement_report_service
        ...
        self._current_filter = StockValuationReportFilterDTO(
            company_id=resolved_company_id or 0,
            as_of_date=initial_filter.date_to if initial_filter else None,
        )
        self._current_report: StockValuationReportDTO | None = None

        self.setObjectName("StockValuationWindow")
        self.setWindowTitle("Stock Valuation Report")
    ...
    def _build_header(self) -> QWidget:
        ...
        title = QLabel("Stock Valuation Report", title_block)
        title.setObjectName("PageTitle")
        title_layout.addWidget(title)

        summary = QLabel(
            "Item-level quantity-on-hand and value using posted inventory movement values and configured valuation metadata where available.",
            title_block,
        )
    ...
    def _build_content_stack(self) -> QWidget:
        ...
        self._stack.addWidget(
            ReportingEmptyState(
                ...
                message="Select an active company before opening the stock valuation report.",
                parent=self,
            )
        )
        self._stack.addWidget(
            ReportingEmptyState(
                title="No Stock Valuation Data",
                message="No posted stock valuation data matched the selected as-of date and filters.",
                parent=self,
            )
        )
        return self._stack
    ...
    def _update_summary(self, report: StockValuationReportDTO | None) -> None:
        self._summary_labels["items"].setText(str(len(report.rows) if report else 0))
        self._summary_labels["quantity"].setText(
            self._fmt_qty(report.total_quantity_on_hand if report else _ZERO_QTY)
        )
        self._summary_labels["value"].setText(
            self._fmt_amount(report.total_inventory_value if report else _ZERO_AMOUNT)
        )

    def _build_filter_from_ui(self, filter_dto: ReportingFilterDTO) -> StockValuationReportFilterDTO:
        return StockValuationReportFilterDTO(
    ...
    def _open_supporting_movement_detail(self, item_id: int) -> None:
        movement_filter = StockMovementReportFilterDTO(
            company_id=self._current_filter.company_id,
            date_from=None,
            date_to=self._current_filter.as_of_date,
            item_id=item_id,
            location_id=self._current_filter.location_id,
        )
        detail = self._movement_service.get_item_detail(movement_filter, item_id)
    ...
    def _on_table_double_clicked(self, row: int, column: int) -> None:  # noqa: ARG002
        ...
        if not isinstance(item_id, int):
            return
        try:
            self._open_supporting_movement_detail(item_id)
