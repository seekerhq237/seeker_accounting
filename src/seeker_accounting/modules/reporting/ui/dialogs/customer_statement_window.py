...
class CustomerStatementWindow(QFrame):
    """Focused child window for customer statements."""
    ...
    def __init__(
        ...
        self._report_service = service_registry.customer_statement_service
        self._customer_service = service_registry.customer_service
        ...
        self._current_report: CustomerStatementReportDTO | None = None

        self.setObjectName("CustomerStatementWindow")
    ...
    def _build_header(self) -> QWidget:
        ...
        layout.setSpacing(4)
        eyebrow = QLabel("Operational Reports | Receivables", card)
        eyebrow.setProperty("role", "caption")
        layout.addWidget(eyebrow)
        title = QLabel("Customer Statement", card)
        title.setObjectName("PageTitle")
        layout.addWidget(title)
        subtitle = QLabel(
            "Formal customer statement with opening balance, period activity, running balance, and source drilldown.",
            card,
        )
    ...
    def _build_controls(self) -> QWidget:
        ...
        customer_label = QLabel("Customer", card)
        customer_label.setProperty("role", "caption")
        layout.addWidget(customer_label)

        self._customer_combo = SearchableComboBox(card)
        self._customer_combo.setFixedWidth(320)
    ...
    def _build_stack(self) -> QWidget:
        ...
        self._stack.addWidget(
            ReportingEmptyState(
                ...
                message="Choose a customer and reporting period to load the statement.",
                parent=self,
            )
        )
        self._stack.addWidget(
            ReportingEmptyState(
                ...
                message="No posted customer activity was found for the selected range.",
                parent=self,
            )
        )
        return self._stack
    ...
    def _build_table_panel(self) -> QWidget:
        ...
        self._table = DataTable(
            columns=(
                ...
                DataTableColumn(key="invoices", title="Invoices"),
                DataTableColumn(key="receipts", title="Receipts"),
        ...
        self._model.setHorizontalHeaderLabels(
            ["Date", "Type", "Document", "Reference", "Description", "Invoices", "Receipts", "Balance"]
        )
    ...
    def _load_customers(self) -> None:
        ...
        if not isinstance(company_id, int) or company_id <= 0:
            self._customer_combo.set_items([], placeholder="Select customer")
            return
        customers = self._customer_service.list_customers(company_id, active_only=False)
        items = [(f"{row.customer_code} - {row.display_name}", row.id) for row in customers]
        search_texts = [f"{row.customer_code} {row.display_name}" for row in customers]
        self._customer_combo.set_items(items, placeholder="Select customer", search_texts=search_texts)
    ...
    def _apply_filter_to_controls(self) -> None:
        ...
        if self._current_filter.customer_id is not None:
            self._customer_combo.set_current_value(self._current_filter.customer_id)

    def _load_report(self) -> None:
        if not isinstance(self._current_filter.customer_id, int):
        ...
        try:
            ...
            show_error(self, "Customer Statement", str(exc))
            return
        except AppError as exc:
            show_error(self, "Customer Statement", str(exc))
            return
        except Exception:
            _log.exception("Customer Statement")
            show_error(self, "Customer Statement", "An unexpected error occurred. See application log for details.")
            return
    ...
    def _bind_rows(self, report: CustomerStatementReportDTO) -> None:
        self._model.removeRows(0, self._model.rowCount())
        for line in report.lines:
            date_item = self._make_item(line.movement_date.strftime("%Y-%m-%d"), user_data=line.journal_entry_id)
            inv_item = self._make_item(self._fmt(line.invoice_amount) if line.invoice_amount else "")
            inv_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            rec_item = self._make_item(self._fmt(line.receipt_amount) if line.receipt_amount else "")
            rec_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            bal_item = self._make_item(self._fmt(line.running_balance))
    ...
    def _update_summary(self, report: CustomerStatementReportDTO | None) -> None:
        if report is None:
            ...
            return
        activity = (report.total_invoices - report.total_receipts).quantize(Decimal("0.01"))
    ...
    def _coerce_filter(
        ...
        if isinstance(initial_filter, OperationalReportFilterDTO):
            return OperationalReportFilterDTO(
                ...
                customer_id=initial_filter.customer_id,
