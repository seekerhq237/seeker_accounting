...
class IasBalanceSheetWindow(QFrame):
    """Focused child window for the IAS/IFRS balance sheet."""
    ...
    def __init__(
        ...
        self._report_service = service_registry.ias_balance_sheet_service
        ...
        self._current_report: IasBalanceSheetReportDTO | None = None
        self._current_template = self._template_service.get_template(None)

        self.setObjectName("IasBalanceSheetWindow")
        self.setWindowTitle("IAS/IFRS Balance Sheet")
    ...
    def _build_header(self) -> QWidget:
        ...
        eyebrow = QLabel("Balance Sheet | IAS / IFRS", title_block)
        eyebrow.setProperty("role", "caption")
        title_layout.addWidget(eyebrow)

        title = QLabel("IAS/IFRS Balance Sheet", title_block)
        title.setObjectName("PageTitle")
        title_layout.addWidget(title)

        summary = QLabel(
            "Locked IAS/IFRS statement of financial position using account metadata and conservative classification policy with drilldown and print preview.",
            title_block,
        )
    ...
    def _build_content_stack(self) -> QWidget:
        ...
        self._stack.addWidget(
            ReportingEmptyState(
                ...
                message="Select an active company before opening the IAS/IFRS balance sheet.",
                parent=self,
            )
        )
        self._stack.addWidget(
    ...
    def _update_warning_band(self, report: IasBalanceSheetReportDTO) -> None:
    ...
    def _update_summary_strip(self, report: IasBalanceSheetReportDTO) -> None:
        self._summary_values["assets"].setText(self._fmt(report.total_assets))
        self._summary_values["liabilities"].setText(self._fmt(report.total_equity_and_liabilities))
    ...
    def _on_export_requested(self) -> None:
        ...
        result = FinancialStatementExportDialog.show_dialog(
            self, "IAS Balance Sheet - Statement of Financial Position",
        )
        if result is None:
            return
        try:
