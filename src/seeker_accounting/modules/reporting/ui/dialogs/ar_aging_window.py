...
class ARAgingWindow(QFrame):
    """Focused child window for AR aging."""
    ...
    def __init__(
        ...
        self._report_service = service_registry.ar_aging_report_service
        ...
        self._current_report: ARAgingReportDTO | None = None

        self.setObjectName("ARAgingWindow")
        self.setWindowTitle("AR Aging")
    ...
    def _build_header(self) -> QWidget:
        ...
        title = QLabel("Accounts Receivable Aging", card)
        title.setObjectName("PageTitle")
        layout.addWidget(title)
        subtitle = QLabel(
            "Posted customer balances grouped into aging buckets with direct drilldown to statement and document detail.",
            card,
        )
    ...
    def _build_stack(self) -> QWidget:
        ...
        self._stack.addWidget(
            ReportingEmptyState(
                ...
                message="Select an active company before opening AR aging.",
                parent=self,
            )
        )
        self._stack.addWidget(
            ReportingEmptyState(
                title="No Receivables Activity",
                message="No posted receivable balances were found for the selected as-of date.",
                parent=self,
            )
        )
        return self._stack
    ...
    def _build_table_panel(self) -> QWidget:
        ...
        self._table.setHorizontalHeaderLabels(["Customer", "Current", "1-30", "31-60", "61-90", "91+", "Total"])
    ...
    def _load_report(self) -> None:
        ...
        try:
            ...
            show_error(self, "AR Aging", str(exc))
            return
    ...
    def _update_warning_band(self, report: ARAgingReportDTO) -> None:
    ...
    def _update_summary(self, report: ARAgingReportDTO) -> None:
        overdue = (
        ...
        self._summary_values["customers"].setText(str(report.customer_count))

    def _bind_rows(self, report: ARAgingReportDTO) -> None:
        self._table.setRowCount(len(report.rows))
        for row_index, row in enumerate(report.rows):
            customer_item = QTableWidgetItem(f"{row.customer_code} - {row.customer_name}")
            customer_item.setData(Qt.ItemDataRole.UserRole, row.customer_id)
            self._table.setItem(row_index, 0, customer_item)
    ...
    def _on_cell_double_clicked(self, row: int, column: int) -> None:
        customer_item = self._table.item(row, 0)
        if customer_item is None:
            return
        customer_id = customer_item.data(Qt.ItemDataRole.UserRole)
        ...
        detail = self._report_service.get_customer_detail(self._current_filter, customer_id, bucket_map.get(column))
