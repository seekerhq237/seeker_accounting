...
_ZERO = Decimal("0.00")
_WINDOWS: list["DepreciationReportWindow"] = []


_log = logging.getLogger(__name__)


class DepreciationReportDetailDialog(QDialog):
    """Period-level depreciation run detail for a single asset."""

    def __init__(
        self,
        service_registry: ServiceRegistry,
        detail_dto: DepreciationReportDetailDTO,
        ...
        self.setWindowTitle(f"Depreciation Detail | {detail_dto.asset_number}")
        ...
        self._table = DataTable(
            columns=(
                DataTableColumn(key="run_no", title="Run #"),
                DataTableColumn(key="run_date", title="Run Date"),
                DataTableColumn(key="period_end", title="Period End"),
                DataTableColumn(key="depreciation", title="Depreciation"),
                DataTableColumn(key="accum_after", title="Accum. After"),
                DataTableColumn(key="carrying_after", title="Carrying After"),
                DataTableColumn(key="posted_je", title="Posted JE"),
            ),
            show_search=False,
            show_count=False,
        ...
        footer = QLabel(
            "Double-click a posted depreciation row to open the journal detail.",
            self,
        )
    ...
    def _build_header(self) -> QWidget:
        ...
        subtitle = QLabel(
            f"Depreciation activity for {self._period_label(self._detail_dto.date_from, self._detail_dto.date_to)}",
            card,
        )
        ...
        self._add_pair(meta_layout, "Opening:", self._fmt(self._detail_dto.opening_accumulated_depreciation))
        self._add_pair(meta_layout, "Period:", self._fmt(self._detail_dto.current_period_depreciation))
        self._add_pair(meta_layout, "Closing:", self._fmt(self._detail_dto.closing_accumulated_depreciation))
    ...
    def _bind_rows(self) -> None:
        ...
        for row in self._detail_dto.rows:
            ...
            dep_item = self._make_item(self._fmt(row.depreciation_amount))
            dep_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            accum_item = self._make_item(self._fmt(row.accumulated_depreciation_after))
            accum_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            carrying_item = self._make_item(self._fmt(row.carrying_amount_after))
            carrying_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
    ...
    def _period_label(date_from, date_to) -> str:
        if date_from is None and date_to is None:
            return "all posted depreciation activity"
        if date_from is None:
            return f"up to {date_to.strftime('%d %b %Y')}" if date_to else "all posted depreciation activity"
        if date_to is None:
...
class DepreciationReportWindow(QFrame):
    """Depreciation report over posted depreciation run truth."""
    ...
    def __init__(
        ...
        self._report_service = service_registry.depreciation_report_service
        ...
        self._current_filter = DepreciationReportFilterDTO(
        ...
        self._current_report: DepreciationReportDTO | None = None

        self.setObjectName("DepreciationReportWindow")
        self.setWindowTitle("Depreciation Report")
    ...
    def _build_header(self) -> QWidget:
        ...
        title = QLabel("Depreciation Report", title_block)
        title.setObjectName("PageTitle")
        title_layout.addWidget(title)

        summary = QLabel(
            "Opening and closing accumulated depreciation, current-period depreciation, and carrying amount by asset.",
            title_block,
        )
    ...
    def _build_summary_strip(self) -> QWidget:
        ...
        for key, title in (
            ("opening", "Opening Accum."),
            ("period", "Period Dep."),
            ("closing", "Closing Accum."),
    ...
    def _build_content_stack(self) -> QWidget:
        ...
        self._stack.addWidget(
            ReportingEmptyState(
                ...
                message="Select an active company before opening the depreciation report.",
                parent=self,
            )
        )
        self._stack.addWidget(
            ReportingEmptyState(
                ...
                message="No posted depreciation activity matched the selected period and filters.",
                parent=self,
            )
        )
        return self._stack
    ...
    def _build_table_panel(self) -> QWidget:
        ...
        self._table = DataTable(
            columns=(
                DataTableColumn(key="asset_code", title="Asset Code"),
                ...
                DataTableColumn(key="opening_accum", title="Opening Accum."),
                DataTableColumn(key="period_dep", title="Period Dep."),
                DataTableColumn(key="closing_accum", title="Closing Accum."),
    ...
    def _load_report(self) -> None:
        ...
        try:
            ...
            show_error(self, "Depreciation Report", str(exc))
            return
        except AppError as exc:
            show_error(self, "Depreciation Report", str(exc))
            return
        except Exception:
            _log.exception("Depreciation Report")
            show_error(self, "Depreciation Report", "An unexpected error occurred. See application log for details.")
            return
    ...
    def _bind_report(self, report: DepreciationReportDTO) -> None:
        self._model.removeRows(0, self._model.rowCount())
        for row in report.rows:
            code_item = self._make_item(row.asset_number, user_data=row.asset_id)
            op_item = self._make_item(self._fmt(row.opening_accumulated_depreciation))
            op_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            pd_item = self._make_item(self._fmt(row.current_period_depreciation))
            pd_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            cl_item = self._make_item(self._fmt(row.closing_accumulated_depreciation))
            cl_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            ca_item = self._make_item(self._fmt(row.carrying_amount))
    ...
    def _update_summary(self, report: DepreciationReportDTO | None) -> None:
        self._summary_labels["opening"].setText(
            self._fmt(report.total_opening_accumulated_depreciation if report else _ZERO)
        )
        self._summary_labels["period"].setText(
            self._fmt(report.total_current_period_depreciation if report else _ZERO)
        )
        self._summary_labels["closing"].setText(
            self._fmt(report.total_closing_accumulated_depreciation if report else _ZERO)
        )
        self._summary_labels["carrying"].setText(
            self._fmt(report.total_carrying_amount if report else _ZERO)
        )

    def _build_filter_from_ui(self, filter_dto: ReportingFilterDTO) -> DepreciationReportFilterDTO:
        return DepreciationReportFilterDTO(
    ...
    def _on_table_double_clicked(self, index) -> None:
        ...
        try:
            ...
            show_error(self, "Depreciation Report", str(exc))
            return
        DepreciationReportDetailDialog.open(self._service_registry, detail, parent=self)
