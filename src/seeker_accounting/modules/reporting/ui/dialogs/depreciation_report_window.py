...
class DepreciationReportDetailDialog(QDialog):
    ...
    def _set_amount(self, row: int, column: int, text: str) -> None:
...
class DepreciationReportWindow(QFrame):
    ...
    def _bind_report(self, report: DepreciationReportDTO) -> None:
        ...
        for row_index, row in enumerate(report.rows):
            ...
            self._table.setItem(row_index, 3, QTableWidgetItem(self._titleize(row.depreciation_method_code)))
            self._set_amount(row_index, 4, self._fmt(row.opening_accumulated_depreciation))
            self._set_amount(row_index, 5, self._fmt(row.current_period_depreciation))
            self._set_amount(row_index, 6, self._fmt(row.closing_accumulated_depreciation))
            self._set_amount(row_index, 7, self._fmt(row.carrying_amount))
            self._table.setItem(row_index, 8, QTableWidgetItem(self._titleize(row.status_code)))
