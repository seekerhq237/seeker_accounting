...
class ContractsPage(QWidget):
    ...
    def _build_action_bar(self) -> QWidget:
        ...
        layout.addWidget(self._change_orders_button)
    ...
    def _build_table_surface(self) -> QWidget:
        ...
        title = QLabel("Contract Directory", top_row)
    ...
    def _activate_selected_contract(self) -> None:
        ...
        choice = QMessageBox.question(
            self,
            "Activate Contract",
            f"Activate contract '{selected.contract_number}'?",
        )
        if choice != QMessageBox.StandardButton.Yes:
            return
        try:
            self._service_registry.contract_service.activate_contract(selected.id)
    ...
    def _cancel_selected_contract(self) -> None:
        ...
        choice = QMessageBox.question(
            self,
            "Cancel Contract",
            f"Cancel contract '{selected.contract_number}'? This cannot be undone.",
        )
        if choice != QMessageBox.StandardButton.Yes:
            return
        try:
            self._service_registry.contract_service.cancel_contract(selected.id)
        except (NotFoundError, ValidationError) as exc:
            show_error(self, "Contracts", str(exc))
        self.reload_contracts(selected_contract_id=selected.id)
