...
class SalesCreditNoteLine(TimestampMixin, Base):
    __tablename__ = "sales_credit_note_lines"
    __table_args__ = (
        UniqueConstraint("sales_credit_note_id", "line_number"),
        Index("ix_sales_credit_note_lines_project_id", "project_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    sales_credit_note_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("sales_credit_notes.id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    sales_credit_note: Mapped["SalesCreditNote"] = relationship("SalesCreditNote", back_populates="lines")
    tax_code: Mapped["TaxCode | None"] = relationship("TaxCode")
    revenue_account: Mapped["Account"] = relationship("Account")
    ...
    tax_details: Mapped[list["SalesCreditNoteLineTax"]] = relationship(
        "SalesCreditNoteLineTax",
        back_populates="sales_credit_note_line",
