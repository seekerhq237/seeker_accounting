...
class SalesOrderDialog(QDialog):
    ...
    def __init__(
        ...
        self._saved_order: SalesOrderDetailDTO | None = None

        is_edit = order_id is not None
        self.setWindowTitle(f"{'Edit' if is_edit else 'New'} Sales Order - {company_name}")
        self.setModal(True)
        apply_window_size(self, "modules.sales.ui.sales.order.dialog.0")
        ...
        install_help_button(self, "dialog.sales_order")

    @property
    def saved_order(self) -> SalesOrderDetailDTO | None:
    ...
    def _build_header_section(self) -> QWidget:
        ...
        # Row 0: Customer | Order Date + Requested Delivery
        ...
        self._delivery_date_edit.setSpecialValueText("No requested date")
        self._delivery_date_edit.setMinimumDate(date(2000, 1, 1))
        self._delivery_date_edit.setDate(date.today())
        dates_layout.addWidget(create_field_block("Requested Delivery (opt.)", self._delivery_date_edit))
        ...
        self._reference_input.setPlaceholderText("Optional SO reference or customer PO number")
        ...
        self._notes_input.setPlaceholderText("Notes or special instructions for this order")
        grid.addWidget(create_field_block("Notes", self._notes_input), 3, 0, 1, 2)

        return card

    def _build_lines_section(self) -> QWidget:
        self._lines_grid = SalesOrderLinesGrid(self._service_registry, self._company_id, self)
    ...
    def _load_order(self) -> None:
        if self._order_id is None:
            return
        try:
            detail = self._service_registry.sales_order_service.get_order(
                self._company_id, self._order_id
            )
        except Exception:
            _log.warning("SO form data load error", exc_info=True)
            return
        ...
        if detail.requested_delivery_date is not None:
            self._delivery_date_edit.setDate(detail.requested_delivery_date)
    ...
    def _handle_submit(self) -> None:
        ...
        try:
            if self._order_id is None:
                ...
                self._saved_order = self._service_registry.sales_order_service.create_draft_order(
                ...
                self._saved_order = self._service_registry.sales_order_service.update_draft_order(
...
class ConvertSalesOrderDialog(QDialog):
    """Collect invoice date, due date, and optional overrides for SO → Invoice conversion."""
    ...
    def __init__(
        ...
        super().__init__(parent)
        self.setWindowTitle(f"Convert Order to Invoice — {order_number}")
        self.setModal(True)
        apply_window_size(self, "modules.sales.ui.sales.order.dialog.1")

        self._result: ConvertSalesOrderCommand | None = None
    ...
    @property
    def result_command(self) -> ConvertSalesOrderCommand | None:
        return self._result
    ...
    def _handle_submit(self) -> None:
        ...
        self._result = ConvertSalesOrderCommand(
