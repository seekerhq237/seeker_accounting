...
from seeker_accounting.modules.sales.dto.sales_order_commands import SalesOrderLineCommand
from seeker_accounting.modules.sales.dto.sales_order_dto import SalesOrderLineDTO
...
class SalesOrderLinesGrid(QFrame):
    """Editable line-items grid for Sales Orders.

    Public API:
        set_lines(lines)
        get_line_commands() -> list[SalesOrderLineCommand]
    ...
    def _load_reference_data(self) -> None:
        try:
            ...
            _log.warning("SO lines: failed to load accounts", exc_info=True)
            self._accounts = []
        ...
        try:
            ...
            _log.warning("SO lines: failed to load tax codes", exc_info=True)
            self._tax_codes = []
            self._tax_rates_by_id = {}
    ...
    def set_lines(self, lines: tuple[SalesOrderLineDTO, ...]) -> None:
        self._updating = True
        try:
    ...
    def get_line_commands(self) -> list[SalesOrderLineCommand]:
        commands: list[SalesOrderLineCommand] = []
