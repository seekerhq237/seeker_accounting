...
class SalesInvoicePrintService:
    ...
    def _invoice_html(
        ...
        if invoice.notes:
            ...
            parts.append(
                f'<p style="font-size:9pt;color:{_P.text_secondary};margin:4px 0;">{h(invoice.notes)}</p>'
            )
    ...
    def _bill_to_section(self, invoice: SalesInvoiceDetailDTO) -> str:
        ...
        left = (
            ...
            f'color:{_P.text_muted};letter-spacing:0.5px;margin-bottom:4px;">Bill To</div>'
            f'<div style="font-weight:700;font-size:11pt;color:{_P.text_primary};margin-bottom:2px;">'
            f'{h(invoice.customer_name)}</div>'
            f'<div style="font-size:9pt;color:{_P.text_muted};">{h(invoice.customer_code)}</div>'
        )
        detail_rows = "".join(
            f'<tr>'
