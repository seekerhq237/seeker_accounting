"""Print and export service for Item Register.

Handles all three output formats (PDF, Word, Excel) for the item listing.
...
class ItemPrintService:
    """Renders the item register to PDF, Word, or Excel."""
    ...
    def print_item_list(
        self,
        company_id: int,
        items: list[ItemListItemDTO],
        result: PrintExportResult,
    ) -> None:
        """Render the item register to the chosen format and path."""
        ...
        if result.format == PrintFormat.PDF:
            html = self._list_html(header, items, result.page_size)
            ...
            doc = self._list_word(header, items, result.page_size, result.orientation)
            doc.save(result.output_path)
        else:
            wb = self._list_excel(header, items, result.page_size, result.orientation)
    ...
    def _list_html(
        ...
        cols = ["Code", "Name", "Type", "UOM", "Active"]
        rows = [
            [
                i.item_code,
                i.item_name,
                i.item_type_code.replace("_", " ").title(),
                i.unit_of_measure_code,
                "Yes" if i.is_active else "No",
            ]
            for i in items
        ]
        total_row = [f"{len(items)} items", "", "", "", ""]
        parts.append(build_data_table(
            cols,
            rows,
            total_row=total_row,
            nowrap_columns={0, 3, 4},
            column_widths={
                0: "16%",
                1: "38%",
                2: "18%",
    ...
    def _list_word(
        ...
        doc.add_document_title("Item Register", subtitle=f"{len(items)} item(s)")
        cols = ["Code", "Name", "Type", "UOM", "Active"]
        rows = [
            [
                i.item_code,
    ...
    def _list_excel(
        ...
        sh = wb.add_sheet("Item Register")
        sh.write_document_header(company, "Item Register")
        sh.write_blank_row()
        sh.write_table_header(["Code", "Name", "Type", "UOM", "Active"])
        for i in items:
            sh.write_table_row([
                i.item_code,
                i.item_name,
        ...
        sh.write_totals_row([f"{len(items)} items", "", "", "", ""])
        sh.write_branded_footer()
        sh.set_column_widths({1: 14, 2: 30, 3: 14, 4: 12, 5: 10})
        return wb
