...
INVENTORY_LOCATION_COLUMNS: tuple[DataTableColumn, ...] = (
...
class _LocationDialog(QDialog):
    def __init__(
        ...
        self._saved: InventoryLocationDTO | None = None

        is_edit = location_id is not None
        self.setWindowTitle(f"{'Edit' if is_edit else 'New'} Inventory Location — {company_name}")
        self.setModal(True)
        apply_window_size(self, "modules.inventory.ui.inventory.locations.page.0")
        ...
        self._code_input.setPlaceholderText("Unique location code")
        form.addRow("Code", self._code_input)

        self._name_input = QLineEdit(card)
        self._name_input.setPlaceholderText("Location name")
    ...
    @property
    def saved(self) -> InventoryLocationDTO | None:
        return self._saved

    def _load(self) -> None:
        try:
            loc = self._service_registry.inventory_location_service.get_inventory_location(
                self._company_id, self._location_id
            )
        except Exception:
            _log.warning("Form data load error", exc_info=True)
            return
        self._code_input.setText(loc.code)
        self._name_input.setText(loc.name)
        self._desc_input.setPlainText(loc.description or "")
        self._active_checkbox.setChecked(loc.is_active)
    ...
    def _submit(self) -> None:
        ...
        try:
            if self._location_id is None:
                self._saved = self._service_registry.inventory_location_service.create_inventory_location(
                    self._company_id, CreateInventoryLocationCommand(code=code, name=name, description=description)
                )
            else:
                self._saved = self._service_registry.inventory_location_service.update_inventory_location(
                    self._company_id,
                    self._location_id,
                    UpdateInventoryLocationCommand(code=code, name=name, description=description, is_active=is_active),
                )
            self.accept()
...
class InventoryLocationsPage(QWidget):
    ...
    def reload(self) -> None:
        ...
        if active_company is None:
            ...
            return
        try:
            rows = self._service_registry.inventory_location_service.list_inventory_locations(
            ...
            show_error(self, "Inventory Locations", f"Could not load data.\n\n{exc}")
            return
        self._populate(rows)
    ...
    def _build_toolbar(self) -> QWidget:
        ...
        title = QLabel('Inventory Locations', card)
        ...
        layout.addStretch(1)
        self._new_btn = QPushButton("New Location", card)
    ...
    def _build_table_surface(self) -> QWidget:
        ...
        self._model = QStandardItemModel(0, len(INVENTORY_LOCATION_COLUMNS), self)
        self._model.setHorizontalHeaderLabels([c.title for c in INVENTORY_LOCATION_COLUMNS])

        self._table = DataTable(
            columns=INVENTORY_LOCATION_COLUMNS,
            show_search=False,
            show_count=False,
            show_density_toggle=True,
            show_column_chooser=True,
            selection_mode="single",
            empty_state_text="No inventory locations to display.",
            parent=card,
        )
    ...
    def _build_empty_state(self) -> QWidget:
        ...
        title = QLabel("No inventory locations", card)
        title.setObjectName("EmptyStateTitle")
        layout.addWidget(title)
        summary = QLabel("Create inventory locations for warehouse and bin tracking.", card)
    ...
    def _build_no_company_state(self) -> QWidget:
        ...
        title.setObjectName("EmptyStateTitle")
        layout.addWidget(title)
        summary = QLabel("Inventory locations are company-scoped.", card)
        summary.setObjectName("PageSummary")
        summary.setWordWrap(True)
        layout.addWidget(summary)
    ...
    def _populate(self, rows: list[InventoryLocationDTO]) -> None:
        self._rows: list[InventoryLocationDTO] = list(rows)
        self._model.removeRows(0, self._model.rowCount())
        for row in rows:
    ...
    def _on_new(self) -> None:
        ...
        if company is None:
            show_error(self, "Inventory Locations", "Select an active company first.")
            return
        dialog = _LocationDialog(self._service_registry, company.company_id, company.company_name, parent=self)
    ...
    def _on_edit(self) -> None:
        ...
        if not (0 <= idx < len(getattr(self, "_rows", []))):
            return
        loc_id = self._rows[idx].id
        dialog = _LocationDialog(
            self._service_registry, company.company_id, company.company_name, location_id=loc_id, parent=self
        )
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.reload()
