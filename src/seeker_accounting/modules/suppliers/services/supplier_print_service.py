"""Print and export service for Supplier Register.

Handles all three output formats (PDF, Word, Excel) for the supplier listing.
...
class SupplierPrintService:
    """Renders the supplier register to PDF, Word, or Excel."""
    ...
    def print_supplier_list(
        ...
        """Render the supplier register to the chosen format and path."""
        ...
        if result.format == PrintFormat.PDF:
            html = self._list_html(header, suppliers, result.page_size)
            ...
            doc = self._list_word(header, suppliers, result.page_size, result.orientation)
            doc.save(result.output_path)
        else:
            wb = self._list_excel(header, suppliers, result.page_size, result.orientation)
    ...
    def _list_html(
        ...
        parts: list[str] = [
            ...
            build_document_title_block(
                "Supplier Register",
                subtitle=f"{len(suppliers)} supplier(s)",
            ),
        ]
        cols = ["Code", "Name", "Group", "Payment Terms", "Country", "Active"]
        rows = [
            [
                s.supplier_code,
                s.display_name,
                s.supplier_group_name or "",
                s.payment_term_name or "",
                s.country_code or "",
                "Yes" if s.is_active else "No",
            ]
            for s in suppliers
        ]
        total_row = [f"{len(suppliers)} suppliers", "", "", "", "", ""]
    ...
    def _list_word(
        ...
        doc.add_document_title("Supplier Register", subtitle=f"{len(suppliers)} supplier(s)")
        cols = ["Code", "Name", "Group", "Payment Terms", "Country", "Active"]
        rows = [
            [
                s.supplier_code,
    ...
    def _list_excel(
        ...
        sh = wb.add_sheet("Supplier Register")
        sh.write_document_header(company, "Supplier Register")
        sh.write_blank_row()
        sh.write_table_header(["Code", "Name", "Group", "Payment Terms", "Country", "Active"])
        for s in suppliers:
            sh.write_table_row([
                s.supplier_code,
                s.display_name,
        ...
        sh.write_totals_row([f"{len(suppliers)} suppliers", "", "", "", "", ""])
        sh.write_branded_footer()
        sh.set_column_widths({1: 14, 2: 30, 3: 18, 4: 18, 5: 10, 6: 10})
        return wb
