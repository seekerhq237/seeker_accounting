...
class SupplierGroupDialog(BaseDialog):
    def __init__(
        self,
        ...
        super().__init__("Supplier Groups", parent, help_key="dialog.supplier_group")
        self.setObjectName("SupplierGroupDialog")
        apply_window_size(self, "modules.suppliers.ui.supplier.group.dialog.0")

        intro_label = QLabel(
            "Keep supplier segmentation compact and reusable so later payable workflows can reference stable supplier groups without widening the master setup unnecessarily.",
            self,
        )
    ...
    def _build_editor_section(self) -> QWidget:
        ...
        self._code_edit.setPlaceholderText("LOCAL")
        grid.addWidget(create_field_block("Code", self._code_edit), 0, 0)

        self._name_edit = QLineEdit(card)
        self._name_edit.setPlaceholderText("Local Suppliers")
    ...
    def reload_groups(self, selected_group_id: int | None = None) -> None:
        try:
            self._groups = self._service_registry.supplier_service.list_supplier_groups(
                self._company_id,
                active_only=False,
            )
            ...
            self._set_error(f"Supplier groups could not be loaded.\n\n{exc}")
            return
    ...
    def _selected_group(self) -> SupplierGroupListItemDTO | None:
    ...
    def _sync_editor_state(self) -> None:
        ...
        try:
            detail = self._service_registry.supplier_service.get_supplier_group(self._company_id, group.id)
    ...
    def _save_group(self) -> None:
        ...
        try:
            if selected_group is None:
                saved_group = self._service_registry.supplier_service.create_supplier_group(
                    self._company_id,
                    CreateSupplierGroupCommand(code=code, name=name),
                )
            else:
                saved_group = self._service_registry.supplier_service.update_supplier_group(
                    self._company_id,
                    selected_group.id,
                    UpdateSupplierGroupCommand(
                        code=code,
            ...
            show_error(self, "Supplier Groups", str(exc))
            self.reload_groups()
            return
    ...
    def _deactivate_group(self) -> None:
        selected_group = self._selected_group()
        if selected_group is None:
            show_info(self, "Supplier Groups", "Select a supplier group to deactivate.")
            return
        if not selected_group.is_active:
            show_info(self, "Supplier Groups", "The selected supplier group is already inactive.")
            return

        choice = QMessageBox.question(
            self,
            "Deactivate Supplier Group",
            f"Deactivate supplier group '{selected_group.name}' ({selected_group.code})?",
        )
        if choice != QMessageBox.StandardButton.Yes:
            return

        try:
            self._service_registry.supplier_service.deactivate_supplier_group(self._company_id, selected_group.id)
