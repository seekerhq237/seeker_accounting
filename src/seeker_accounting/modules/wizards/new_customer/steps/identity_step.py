...
class IdentityStep(WizardStep):
    key = "identity"
    title = "Identity"
    subtitle = "Customer code, display name, and group."
    ...
    def build_widget(self, parent: QWidget | None = None) -> QWidget:
        ...
        self._code.setPlaceholderText("e.g. CUST-001")
        self._code.setMaxLength(40)
        form.addRow(QLabel("Customer code:", root), self._code)
    ...
    def load(self, context: WizardContext, state: WizardState) -> None:
        if self._group is not None and self._group.count() == 0:
            ...
            for g in context.service_registry.customer_service.list_customer_groups(
                company_id, active_only=True
            ):
                self._group.addItem(f"{g.code} \u2014 {g.name}", g.id)
            prior = state.get(K.KEY_CUSTOMER_GROUP_ID)
            if isinstance(prior, int):
                idx = self._group.findData(prior)
                if idx >= 0:
                    self._group.setCurrentIndex(idx)
        if self._code is not None and state.get(K.KEY_CUSTOMER_CODE):
            self._code.setText(str(state[K.KEY_CUSTOMER_CODE]))
    ...
    def write_back(self, state: WizardState) -> None:
        if self._code is not None:
            state[K.KEY_CUSTOMER_CODE] = self._code.text().strip()
        ...
        if self._group is not None:
            ...
            state[K.KEY_CUSTOMER_GROUP_ID] = int(data) if isinstance(data, int) else None

    def validate(self, context: WizardContext, state: WizardState) -> StepValidationResult:
        if not state.get(K.KEY_CUSTOMER_CODE):
            return StepValidationResult.fail("Customer code is required.")
        if not state.get(K.KEY_DISPLAY_NAME):
            return StepValidationResult.fail("Display name is required.")
        return StepValidationResult.ok()

    def preview(self, context: WizardContext, state: WizardState) -> str | None:
        return f"{state.get(K.KEY_CUSTOMER_CODE) or '(no code)'} \u2014 {state.get(K.KEY_DISPLAY_NAME) or ''}".strip()
