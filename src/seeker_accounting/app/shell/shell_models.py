...
SIDEBAR_MODULES: tuple[SidebarModule, ...] = (
    ...
    SidebarModule(
        ...
        children=(
            ...
            SidebarChild(nav_id=nav_ids.TAX_PROFILE, label="Tax Profile"),
            SidebarChild(nav_id=nav_ids.TAX_COMPLIANCE, label="Tax Compliance"),
            SidebarChild(nav_id=nav_ids.TAX_DASHBOARD, label="Tax Dashboard"),
            SidebarChild(nav_id=nav_ids.TAX_AUDIT_TRAIL, label="Tax Audit Trail"),
            SidebarChild(nav_id=nav_ids.WITHHOLDING_CERTIFICATES, label="Withholding Certificates"),
...
NAVIGATION_SECTIONS: tuple[NavigationSection, ...] = (
    ...
    NavigationSection(
        ...
        items=(
            ...
            NavigationItem(
                nav_id=nav_ids.TAX_PROFILE,
                label="Tax Profile",
                section_label="Accounting Setup",
                description="NIU, regime, VAT, DSF settings",
                module_code=ModuleCode.TAX_PROFILE,
            ),
            NavigationItem(
                nav_id=nav_ids.TAX_COMPLIANCE,
                label="Tax Compliance",
                section_label="Accounting Setup",
                description="VAT obligations, returns, payments, DSF export",
                module_code=ModuleCode.TAX_COMPLIANCE,
            ),
            NavigationItem(
                nav_id=nav_ids.TAX_DASHBOARD,
                label="Tax Dashboard",
                section_label="Accounting Setup",
                description="Compliance KPIs and upcoming due dates",
                module_code=ModuleCode.TAX_DASHBOARD,
            ),
            NavigationItem(
                nav_id=nav_ids.TAX_AUDIT_TRAIL,
                label="Tax Audit Trail",
                section_label="Accounting Setup",
                description="Chronological audit events for the taxation module",
                module_code=ModuleCode.TAX_AUDIT_TRAIL,
            ),
            NavigationItem(
                nav_id=nav_ids.WITHHOLDING_CERTIFICATES,
                label="Withholding Certificates",
                section_label="Accounting Setup",
                description="Inbound and outbound withholding-tax certificate register",
                module_code=ModuleCode.WITHHOLDING_CERTIFICATES,
            ),
            NavigationItem(
...
PLACEHOLDER_PAGES = {
    ...
    nav_ids.TAX_PROFILE: PlaceholderPageModel(
        nav_id=nav_ids.TAX_PROFILE,
        title="Tax Profile",
        summary="The company's tax-compliance identity — NIU, regime, VAT liability, and DSF filing setup — kept in a single calm form.",
        status_code=StatusCode.PLACEHOLDER,
        available_now=(
            "View profile",
            "Edit dialog with regime, VAT, DSF settings",
        ),
        next_slice=(
            "DSF export from this surface",
            "Compliance calendar integration",
        ),
    ),
    nav_ids.TAX_COMPLIANCE: PlaceholderPageModel(
        nav_id=nav_ids.TAX_COMPLIANCE,
        title="Tax Compliance",
        summary="Operational workspace for tax obligations, returns, and payments, with DSF export driven from posted accounting truth.",
        status_code=StatusCode.PLACEHOLDER,
        available_now=(
            "Generate monthly VAT obligations",
            "Draft and file VAT returns",
            "Record tax payments",
            "Run DSF readiness check and Excel export",
        ),
        next_slice=(
            "Compliance calendar with due-date dashboard",
            "CIT installments and SME deadlines",
        ),
    ),
    nav_ids.TAX_DASHBOARD: PlaceholderPageModel(
        nav_id=nav_ids.TAX_DASHBOARD,
        title="Tax Dashboard",
        summary="Consolidated tax-compliance snapshot for the active fiscal year — counts, money totals, and upcoming due dates.",
        status_code=StatusCode.PLACEHOLDER,
        available_now=(
            "Obligations breakdown (open, overdue, paid, cancelled)",
            "Per-tax-type counts",
            "VAT settlement gap (filed but not yet settled)",
            "Top upcoming obligations",
        ),
        next_slice=(),
    ),
    nav_ids.TAX_AUDIT_TRAIL: PlaceholderPageModel(
        nav_id=nav_ids.TAX_AUDIT_TRAIL,
        title="Tax Audit Trail",
        summary="Chronological record of every audit event raised by the taxation module.",
        status_code=StatusCode.PLACEHOLDER,
        available_now=(
            "Browse taxation audit events",
            "Filter by event type and date range",
        ),
        next_slice=(),
    ),
    nav_ids.WITHHOLDING_CERTIFICATES: PlaceholderPageModel(
        nav_id=nav_ids.WITHHOLDING_CERTIFICATES,
        title="Withholding Certificates",
        summary="Compliance register for inbound (received) and outbound (issued) withholding-tax certificates, with per-direction totals for the selected period.",
        status_code=StatusCode.PLACEHOLDER,
        available_now=(
            "Record a new certificate",
            "Edit or void an existing certificate",
            "Filter by direction, status, and date range",
            "View per-direction totals for the active period",
        ),
        next_slice=(
            "Auto-link outbound certificates to supplier-payment journal entries",
            "Inbound offset against tax liabilities",
        ),
    ),
