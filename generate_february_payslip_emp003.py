"""
Generate February 2026 payslip for EMP003 (Jean-Pierre Tchoua).

Creates a fresh company (unique name), runs February 2026 payroll for all 4 demo employees,
and exports only EMP003's payslip to the project root.

Usage:
    python generate_february_payslip_emp003.py
"""
...
def main() -> None:
    ...
    print("=" * 65)
    print("  Seeker Accounting — February 2026 Payslip Generator (EMP003)")
    print("=" * 65)
    ...
    target = next((re for re in run_employees if re.employee_number == "EMP003"), None)
    if not target:
        print("ERROR: EMP003 not found in run employees.")
        return
    if target.status_code != "included":
        print(f"ERROR: EMP003 status is {target.status_code}, not included.")
        return
