"""
Generate January 2026 payslip for EMP001 (Amadou Koné).

Creates a fresh company (unique name), runs January 2026 payroll for all 4 demo employees,
and exports only EMP001's payslip to the project root.

Usage:
    python generate_january_payslip.py
"""
...
def main() -> None:
    ...
    print("=" * 65)
    print("  Seeker Accounting — January 2026 Payslip Generator")
    print("=" * 65)
    ...
    print("\nCreating January 2026 payroll run...")
    run = reg.payroll_run_service.create_run(
        cid,
        CreatePayrollRunCommand(
            period_year=2026,
            period_month=1,
            run_label=f"January 2026 Payroll {ts}",
            currency_code="XAF",
    ...
    emp1 = next((re for re in run_employees if re.employee_number == "EMP001"), None)
    if not emp1:
        print("ERROR: EMP001 not found in run employees.")
        return
    if emp1.status_code != "included":
        print(f"ERROR: EMP001 status is {emp1.status_code}, not included.")
        return
    reg.payroll_run_service.approve_run(cid, run.id)
    print(f"Run approved.")
    safe_name = emp1.employee_display_name.replace(" ", "_").replace("/", "_")
    filename = f"payslip_JAN2026_{emp1.employee_number}_{safe_name}.pdf"
    output_path = str(project_root / filename)
    result = reg.payroll_export_service.export_payslip_pdf(
        company_id=cid,
        run_employee_id=emp1.id,
